...
Notes:
  ...
  - The KSU driver version is pinned via ksu_version.override.mk to match the
    prebuilt ReSukiSU manager APK's versionCode (default 34990). Override with
    KSU_VERSION_PIN=<code> to use a different prebuilt APK.
EOF
}
...
write_version_override() {
    local override="${KSU_KERNEL_DIR}/ksu_version.override.mk"
    local version="${KSU_VERSION_PIN:-34990}"
    cat > "${override}" <<EOF
# Auto-generated by setup.sh. Pins KSU_VERSION to match the prebuilt
# ReSukiSU APK (versionCode=${version}). The prebuilt APK was built with
# the legacy version formula (30000 + commit_count + 700), while the
# current Kbuild uses (40000 + commit_count - 2815). This override bridges
# the difference so driver and manager report the same version.
# Only KSU_VERSION is pinned; KSU_VERSION_FULL uses the natural git formula.
KSU_VERSION := ${version}
EOF
    echo "[+] Wrote version override: KSU_VERSION=${version} (matches prebuilt APK)"
}
...
report_driver_state() {
    ...
    if [ -f "${KSU_KERNEL_DIR}/ksu_version.override.mk" ]; then
        echo "[state] version override: present"
    else
        echo "[state] version override: absent"
    fi
}
